...
function Parser:parse(expression)
  ...
  if self.tokens.cur.type ~= 'eof' then
    throw(parser, 'Encountered an unexpected "' .. self.tokens.cur.type
...
local parselets = {
  ...
  nud_identifier = function(parser)
  ...
  nud_quoted_identifier = function(parser)
  ...
  nud_current = function(parser)
    parser.tokens:next()
    return {type = 'current'}
  end,

  --- Parses a literal token (e.g., `foo`)
  nud_literal = function(parser)
    local token = parser.tokens.cur
    parser.tokens:next()
  ...
  led_lbracket = function(parser, left)
  ...
  led_flatten = function(parser, left)
    parser.tokens:next()
    return {
      type = 'projection',
      from = 'array',
  ...
  led_lparen = function(parser, left)
    local args = {}
    local name = left.key
    parser.tokens:next()
  ...
  led_filter = function(parser, left)
    parser.tokens:next()
  ...
  led_comparator = function(parser, left)
    local token = parser.tokens.cur
    parser.tokens:next()
